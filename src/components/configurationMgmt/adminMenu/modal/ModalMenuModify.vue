<template>
  <Modal>
    <template #modal-header>
      <p class="title">관리자 메뉴 메뉴수정</p>
      <span class="subtitle">관리자 메뉴를 수정하는 영역입니다.</span>
    </template>
    <template #modal-body>
      <div class="contnet">
        <table class="table_04">
          <tr>
            <th>구분</th>
            <td>
              <select class="select_02 select_04 select300 corSelect" v-model="modifyData.구분">
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const modifyData = computed(() => props.detailData);
    return { modifyData };
  },
});
</script>
