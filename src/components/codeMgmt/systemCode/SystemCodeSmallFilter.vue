<template>
  ...
  <ul class="tabs">
    <li class="tab-link current" data-tab="tab-1">상태코드</li>
  </ul>
  <ul class="inputList">
    <li class="margin0">
      <div class="listBox">
        <select class="select_01 select160" v-model="filterData.검색조건">
          ...
          <option value="업무코드">업무 코드</option>
  ...
  <ModalSystemSmallCodeWrite v-if="isModal" @close="isModalFnc" />
</template>
<script>
...
export default defineComponent({
  components: {
    ...
    ModalSystemSmallCodeWrite: defineAsyncComponent(() =>
      import('./modal/ModalSystemSmallCodeWrite.vue'),
    ),
